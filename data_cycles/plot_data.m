    ...
    disf=person{k};
        for j=1:size(disf,2)
            ...
            %reshape the data to shape 25xT, where T - number of frames
             joints_data=reshape(data', 75, N/25);
             [ign t]=size(joints_data);
             left_feet_Z= joints_data(14*3+3,:);
           
            coeff_data=size(left_feet_Z,2);
             left_feet_Z = resample( left_feet_Z,coeff_res,coeff_data);
             
             right_feet_Z=joints_data(19*3+3,:);
             coeff_data=size(right_feet_Z,2);
             right_feet_Z = resample(right_feet_Z,coeff_res,coeff_data);
                 % ankle, hip, knee,  foot 
               %PlotData(data);
        %    XYZ=reshape(right_feet_joints, 3, 4*t)';
            % 
